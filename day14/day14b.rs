...
struct Grid {
    ...
    data: Vec<VecDeque<Cell>>
}

impl Grid {
    fn new(left: i64, right: i64, height: i64) -> Grid {
        let grid_data = vec![
            std::iter::repeat(Cell::Air).take((right - left) as usize).collect(); (height + 2) as usize];
        ...
        // Make floor
        for x in left..right {
            res.set_at(x, height + 1, Cell::Rock);
        }
    ...
    fn in_grid(&self, x: i64, y: i64) -> bool {
        (y < (self.data.len() as i64)) && (x >= self.left) && ((x - self.left) < (self.data[y as usize].len() as i64))
    }

    // Return whether the unit came to rest at the source
        ...
        loop {
            ...
            match self.get_at(sand.x, sand.y + 1) {
                ...
                Cell::Rock | Cell::Sand => {
                    ...
                    if !self.in_grid(sand.x - 1, sand.y + 1) {
                        // Insert a new column
                        self.left -= 1;
                        let height = self.height() as usize;
                        for i in 0..height {
                            self.data[i].push_front(
                                if i == (height - 1) { Cell::Rock } else { Cell::Air }
                            );
                        }
                    }
